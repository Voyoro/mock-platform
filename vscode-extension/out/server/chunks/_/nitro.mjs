...
function setMockConfig(id, config) {
  mockMap.set(id, config);
  saveToFile();
}
function getMockConfig(id) {
...
class MockDB {
  constructor() {
    __publicField(this, "dbMap", /* @__PURE__ */ new Map());
  }
  getDB(mockId) {
    if (!this.dbMap.has(mockId)) {
      const dbFile = join$1(process.cwd(), "DB", `${mockId}.db`);
  ...
  list(mockId, query = {}, apiConfig) {
    ...
    return new Promise((resolve, reject) => {
      db.findOne({ path: query.path }, (err, doc) => {
        ...
        if (data !== null && (apiConfig == null ? void 0 : apiConfig.rowData) && apiConfig.count && Array.isArray(data[apiConfig.rowData])) {
          const filteredList = this.filterAndPaginate(data[apiConfig.rowData], query);
          resolve({
            ...data,
            [apiConfig.rowData]: filteredList,
            [apiConfig.count]: data[apiConfig.rowData].length
            // 保持 total
          });
          return;
        }
  ...
  filterAndPaginate(list, query, apiConfig) {
    var _a, _b;
    const currentPage = (_a = apiConfig == null ? void 0 : apiConfig.page) != null ? _a : "currentPage";
    const numberPerPage = (_b = apiConfig == null ? void 0 : apiConfig.size) != null ? _b : "numberPerPage";
    let filtered = list;
...
function loadYaml(yamlPath) {
  ...
  setMockConfig("admin", doc);
}
function initMockLoader() {
...
async function mockLoaderPlugin(nitroApp) {
  initMockLoader();
...
const _RsT00B = eventHandler((event) => {
...
const _lazy_1298a_ = () => import('../routes/mock/_mockId/_...slug_.mjs');
const _lazy_xwyYhD = () => import('../routes/upload.post.mjs');

const handlers = [
  { route: '', handler: _RsT00B, lazy: false, middleware: true, method: undefined },
  { route: '/mock/:mockId/**:slug', handler: _lazy_1298a_, lazy: true, middleware: false, method: undefined },
  { route: '/upload', handler: _lazy_xwyYhD, lazy: true, middleware: false, method: "post" }
];
...
export { trapUnhandledNodeErrors as a, useNitroApp as b, defineEventHandler as c, destr as d, getMockConfig as e, setResponseStatus as f, getRouterParam as g, getQuery as h, setHeader as i, readMultipartFormData as j, setMockConfig as k, mockDB as m, randomUUID as r, setupGracefulShutdown as s, toNodeListener as t, useRuntimeConfig as u };
